...
export const getLevels = async (req: Request, res: Response): Promise<void> => {
  try {
    const { page = 1, limit = 12, difficulty, search, sort = 'newest' } = req.query;
    const pageNum = Number(page);
    const limitNum = Number(limit);

    const filter: Record<string, unknown> = {};
    if (difficulty && difficulty !== 'all') filter['meta.difficulty'] = difficulty;
...
export const getLevelByCode = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const createLevel = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!meta && req.body.name) {
      const { name, description, difficulty = 'medium', blueprint } = req.body;
      if (!blueprint?.grid || !blueprint?.pieces) {
        res.status(400).json({ message: 'blueprint.grid and blueprint.pieces are required' });
        return;
      }
      grid = blueprint.grid;
    ...
    if (!meta || !grid || !pieces) {
      res.status(400).json({ message: 'name/blueprint or meta/grid/pieces are required' });
      return;
    }
    ...
    while (await Level.findOne({ code })) {
      ...
      attempts++;
      if (attempts > 100) {
        res.status(500).json({ message: 'Failed to generate unique code' });
        return;
      }
    }
...
export const updateLevel = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const level = await Level.findOne({ code: req.params.code.toUpperCase() });
...
export const getUserLevels = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const downloadLevel = async (req: Request, res: Response): Promise<void> => {
  try {
