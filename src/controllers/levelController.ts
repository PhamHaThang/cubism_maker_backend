...
const approvedReviewFilter = { $in: ["approved"] };

const isReviewApproved = (value: unknown): boolean => {
...
export const getLevels = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const filter: Record<string, unknown> = {
            status: { $in: ["public"] },
            reviewStatus: approvedReviewFilter,
            isMainMenu: { $ne: true },
        };
        ...
        if (search)
            filter["meta.name"] = {
                $regex: escapeRegex(search as string),
                $options: "i",
            };
...
export const getMainMenuLevels = async (
    ...
    try {
        ...
        const limitNum = Math.min(200, Math.max(1, Number(limit) || 8));
...
export const getLevelByCode = async (
    ...
    try {
        ...
        const isOwner = req.userId && level.author?.toString() === req.userId;
        const isAdmin = Boolean(req.userIsAdmin);
        if (!isReviewApproved(level.reviewStatus) && !isOwner && !isAdmin) {
            res.status(403).json({ message: "Level is pending review" });
            return;
        }
...
export const createLevel = async (
    ...
    try {
        ...
        if (req.body.isMainMenu !== undefined && isAdmin) {
        ...
        const latestLevel = await Level.findOne({
            code: { $regex: `^${prefix}\\d+$` },
        }).sort({ createdAt: -1 });
        ...
        if (isAdmin) {
            levelData.reviewedAt = now;
            levelData.reviewedBy = req.userId;
        }
        if (order !== undefined) {
    ...
    try {
        ...
        const shouldRequireReview =
            ...
            (meta ||
                ...
                level.status === "public" ||
                nextStatus === "public");
        if (shouldRequireReview && level.reviewStatus !== "pending") {
...
export const getUserLevels = async (
    ...
    try {
        ...
        const isOwner = req.userId && req.userId === req.params.userId;
        const isAdmin = Boolean(req.userIsAdmin);
        if (!isOwner && !isAdmin) {
            filter.status = { $in: ["public"] };
            filter.reviewStatus = approvedReviewFilter;
        }
        if (difficulty && difficulty !== "all") {
            filter["meta.difficulty"] = difficulty;
        }
            ...
            if (isOwner || isAdmin) {
                filter.status =
                    parsedStatus === "public"
                        ? { $in: ["public", "publish"] }
                        : "private";
            } else if (parsedStatus === "public") {
                filter.status = { $in: ["public", "publish"] };
            }
        }
        if (reviewStatus && reviewStatus !== "all" && (isOwner || isAdmin)) {
            ...
            filter.reviewStatus = String(reviewStatus);
        }
